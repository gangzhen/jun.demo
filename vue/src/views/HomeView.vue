    ...
    <el-container>
      <el-header style="font-size: 12px; border-bottom: 1px solid #ccc; line-height: 60px; display: flex">
        ...
        <el-dropdown style="width: 70px; cursor: pointer">
          <span>yaojun</span><i class="el-icon-arrow-down" style="margin-left: 5px"></i>
      ...
      <el-main>
        ...
        <el-form :inline="true" :model="searchForm" class="demo-form-inline">
          ...
          <el-form-item>
            ...
            <el-button type="danger" v-show="batchDeleteFlag" @click="handleBatchDelete">选择删除 <i class="el-icon-delete-solid"></i></el-button>
            <el-popconfirm
                class="ml-10"
...
export default {
  ...
  methods: {
    ...
    searchRecord() {
      //分页查询
      request.post("/bodyData/page", this.searchForm, {
        params: {
    ...
    deleteRecord(id) {
      //删除
      request.delete("/bodyData/" + id).then(res => {
        console.log(res)
        if (res) {
    ...
    batchDeleteRecord() {
      ...
      if (this.batchRecordIds.length === 0) {
        ...
        request.delete("/bodyData/del/batch", {data: this.batchRecordIds}).then(res => {
          console.log(res)
          if (res) {
            this.$message.success('删除成功')
          } else {
    ...
    submitRecord(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.dialogFormVisible = false
          request.post("/bodyData", this.infoForm).then(res => {
            console.log(res)
            if (res) {
